-- =============================================================
-- 14_note_detail_rpc_is_client: 明細付き Be:note 作成 RPC に is_client を追加
-- =============================================================
-- DM 写真（photo）を顧客が送信できるようになったため、明細ノードにも
-- 送信者識別（is_client）を保存する必要がある。create_note_with_details に
-- payload.is_client を追加し、ノード insert で設定する。
-- 未指定（item / discount / staff 発 photo）は NULL のまま。
-- =============================================================
...
declare
  ...
  v_isclient  boolean  := (payload ->> 'is_client')::boolean;  -- NULL 可（item/discount は未指定）
  v_details   jsonb    := payload -> 'details';
  v_note      uuid;
  d           jsonb;
begin
  -- 明細ノード（head の子）。
  insert into t_be_note (p_note_id, note_type, salon_id, client_id, responsible, future_flg, is_client)
  values (v_parent, v_type, v_salon, v_client, v_resp, v_future, v_isclient)
