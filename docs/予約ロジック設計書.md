...
- `t_reservation` は `delete_flg` を持たず親 `t_be_note.delete_flg` に従う。予約ノートを論理削除する際は **`status` を `cancelled` に更新する**こと（さもないと `confirmed` のまま排他制約が枠をブロックし続ける）
...
CREATE TABLE t_staff (
    ...
    user_id         UUID  UNIQUE REFERENCES auth.users(id),  -- Supabase Auth 連携
    salon_id        UUID  NOT NULL REFERENCES t_salon,
...
-- 分割シフト（中抜け）を許容するため一意制約は設けない。時間帯重複は排他制約で防ぐ（DB設計書 参照）
```
