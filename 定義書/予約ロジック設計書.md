...
通知手段はログイン設計と連動するため、ログイン方式確定後に定義する。（未解決事項 No.1）
...
### 5-3. t_client_salon（顧客×サロン中間テーブル）
...
### 5-9. t_staff_skill（スタッフ可能タスク）

アシスタントのタスク制限をDB管理する。マスタメンテ画面（SON9）から変更できる。
...
| `allergy` | キー末尾スペース除去 |
| `client_id` 体系 | `CID...` に統一（`UID...` との不整合解消） |
...
### 6-4. t_menu（スタッフ参照を staff_id に変更）
...
ロール    : customer / staff / admin（JWT クレームに含める）
日時形式  : ISO 8601 UTC（例: 2026-06-01T01:00:00Z）
```
...
Response : 201 / 409 DOUBLE_BOOKING
```
...
Body:
{
  ...
  "menu_master_id" : 2,
  "staff_id"       : "STF0000001"  // 任意
}
Response : 201 { "reservation_id": "...", "status": "requested" }
```
...
Body:
{
  ...
  "main_menu"         : "カット",
  "menu_list"         : [...],
  "version_no"        : 1   // 楽観ロック。DBの値と不一致なら 409 VERSION_CONFLICT
}
Response : 200 / 409
```
...
Auth : staff / admin（customer は cancelled のみ）
Body : { "status": "confirmed", "reason": "..." }
Response : 200 / 409 DOUBLE_BOOKING
```
...
sequenceDiagram
    ...
    alt 重複なし
        ...
        API->>DB: COMMIT
        API-->>Staff: 200
        Note over API,C: 通知手段は別途定義（未解決事項 No.1）
    else 重複あり
        API->>DB: ROLLBACK
...
| No. | 項目 | 状態 | 内容 |
|---|---|---|---|
| 1 | 通知手段 | 保留 | ログイン方式（フェーズ1）実装後に定義する。候補：FCM Push / メール |
| 2 | 外部決済連携 | 将来 | MVP は手動会計。連携時に `t_payment` テーブルを追加 |
| 3 | キャンセル手数料 | 将来 | 当日キャンセル不可はデフォルト設定済み。手数料徴収は外部決済連携と合わせて検討 |
| 4 | Instagram 投稿機能 | 将来 | Instagram Login の基盤を活用。`instagram_content_publish` 権限追加で対応 |
| 5 | LINE ログイン | フェーズ2 | フェーズ1（独自・Google・Instagram）完了後に追加 |
