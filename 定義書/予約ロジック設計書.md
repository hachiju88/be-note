...
- 当面は1店舗運用。`salon_id` は最初から全テーブルに持たせ、多店舗展開時にグループマスタ＋中間テーブルを追加する
...
| No. | 項目 | 内容 |
|---|---|---|
| 1 | ログイン方式・通知手段 | ログイン方式（LINE / Google / Instagram / Be:note独自）未決定。通知手段はこれに連動 |
| 2 | 技術スタック | BaaS（Supabase）vs フルカスタム未決定 |
| 3 | 会計・税計算 | 消費税率・ポイント・複数決済手段のモデルなし |
| 4 | キャンセルポリシー | 何時間前まで無料キャンセル可能か |
| 5 | 写真保存先 | `"path":"C:/XXX"` はローカルパス → クラウドストレージ URL に変更必要 |
| 6 | 多店舗展開 | グループ会社マスタ＋中間テーブルの詳細設計 |
